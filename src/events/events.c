...
static int	key_handler_3(int keysym, t_game *r)
{
	...
	{
		if (r->dist_ratio < 1.0 && r->dist_ratio > .5)
			r->iters += .125 * SQ(1 - r->dist_ratio) * SQ(r->r * r->zoom) * (double)r->sides * sin(2 * M_PI / (double)r->sides);
		else if (r->dist_ratio <= .5)
			r->iters += .125 * SQ(r->dist_ratio) * SQ(r->r * r->zoom) * (double)r->sides * sin(2 * M_PI / (double)r->sides);
		else// (r->dist_ratio > 1)
			r->iters += .125 * SQ(r->dist_ratio - 1) * SQ(r->r * r->zoom * r->ratio_change) * (double)r->sides * sin(2 * M_PI / (double)r->sides);
		r->iters_change += 1;
	}
	else if (keysym == MINUS)
	{
		//r->iters -= SQ(r->dist_ratio) * SQ(r->r * r->zoom) * r->area_factor;//old
		if (r->dist_ratio < 1.0 && r->dist_ratio > .5)
			r->iters -= .125 * SQ(1 - r->dist_ratio) * SQ(r->r * r->zoom) * (double)r->sides * sin(2 * M_PI / (double)r->sides);
		else if (r->dist_ratio <= .5)
			r->iters -= .125 * SQ(r->dist_ratio) * SQ(r->r * r->zoom) * (double)r->sides * sin(2 * M_PI / (double)r->sides);
...
void adjust_ratio(t_game *r, double new_ratio)
{
	...
	if (old > 1 && old <= 2 && old != 1)//1st undo prev
		r->zoom *= r->ratio_change;

	if (new_ratio == 2)
		r->ratio_change = 600;
	/* else if (new_ratio < .5)
		r->ratio_change = new_ratio / (new_ratio - 1); */
	else
		r->ratio_change = new_ratio / (2 - new_ratio);
...
static int	key_handler_2layer(int keysym, t_game *r)//need to make new iters adjustments??
{
	if (keysym == PAD_8)
		r->ratio_start += .05;
	else if (keysym == PAD_2)
	...
	else if (keysym == QUES)//ghost center for col
	{
...
void	print_guide(void)//in prog
{
	...
	ft_putstr_color_fd(1, "╠╣ [F11]      => Toggle between RGB and CMYK       ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣                                                 ╠╣\n", BRIGHT_BLUE);
...
//	ft_putstr_color_fd(1, "╠╣ [NUM 1]    => Switch to/from color wheel colors ╠╣\n", BRIGHT_BLUE);
	...
	ft_putstr_color_fd(1, "╠╣ [,]   	 =>        ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [.]   	 =>        ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [?]  	 	 => Toggle Jump to center             ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [´] 	     => Toggle Jump to sides              ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣                                                 ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣-------------------------------------------------╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣                                                 ╠╣\n", BRIGHT_BLUE);
	...
	ft_putstr_color_fd(1, "╠╣                                                 ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [NPAD +]   => increase supersample level by 1   ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [NPAD -]   => decrease supersample level by 1   ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [+]        => 								  ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [-]        => 								  ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [Y] - [M]  => change to diff color sets part 2  ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [´] 	     => toggle ghost sides (color verts)  ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣ [?]        => toggle thru ghost center settings ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╠╣                                                 ╠╣\n", BRIGHT_BLUE);
	ft_putstr_color_fd(1, "╚╩═════════════════════════════════════════════════╩╝\n", BRIGHT_BLUE);
}
//╔ ╗ ╚ ╝ ═ ║, ╦ ╩ ╠ ╣, ➡ ⬅ ⬆ ⬇
