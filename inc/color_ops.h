...
static inline unsigned int set_wheel_center(t_game *r, int x, int y, int curr, int cenx, int ceny, bool g)
{
	...
	//radial
	//color = get_color_source(col_x, col_y, r->wheel->colors, 360, r->color_rot + rot_adjust, 1);
	
	double 			factor;
	(!r->supersample) ? (factor = 1): (factor = 1 / (double)r->s_kernel);
	//spiral
	color = get_color_source_spiral(col_x, col_y, r->wheel->colors, 360, r->color_rot + rot_adjust, .0066 * factor);
...
static inline void	color_option(t_game *r, int v, double x, double y, int **vertices)
{
	unsigned int		color;
	...
	else if (r->color_op == 1)//color scource(wheel(s))
	{
		...
		//	color = get_color_source_spiral(col_x, col_y, r->wheel->colors, 360, r->color_rot, .01 * factor);
			color = get_color_source(col_x, col_y, r->wheel->colors, 360, r->color_rot, r->wheel->freq);
		}
		else if (r->color_depth == 1)
	...
	put_pixel((int)x, (int)y, r, color);		//interp txs, maybe use cubic interp instead
}
#endif//COLOR_OP_H